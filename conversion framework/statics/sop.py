...
from utils import ExpandTemporalDim,MyConv,MyLinear,MyPass
...
class SOPMonitor(BaseMonitor):
    def __init__(self, net: nn.Module,step_mode='s',T=8,coding_type='diff_rate',neuron_name='o.0'):
        super().__init__()
        ...
        if "H_MT" in self.neuron_name:
            for name, m in net.named_modules():
                if isinstance(m, MyConv):
                    self.monitored_layers.append(name)
                    self.name_records_index[name] = []
                    self.hooks.append(m.register_forward_hook(self.create_hook_Myconv(name,step_mode)))
                elif isinstance(m, MyLinear):
                    self.monitored_layers.append(name)
                    self.name_records_index[name] = []
                    self.hooks.append(m.register_forward_hook(self.create_hook_Mylinear(name,step_mode)))
        else:
            for name, m in net.named_modules():
    ...
    def cal_sop_Myconv_s(self, x: tuple, m: nn.Conv2d):
        x_spike = x[1]
        with torch.no_grad():
            sop = (torch.nn.functional.conv2d(x_spike, torch.ones_like(m.weight), None, m.stride,m.padding, m.dilation, m.groups)).sum()
            tot = (torch.nn.functional.conv2d(torch.ones_like(x_spike), torch.ones_like(m.weight), None, m.stride,m.padding, m.dilation, m.groups)).sum()
    ...
    def create_hook_Myconv(self, name, step_mode):
        if step_mode=='s':
            def hook_s(m: nn.Conv2d, x: tuple, y: torch.Tensor):
                self.name_records_index[name].append(self.records.__len__())
                self.records.append(self.cal_sop_Myconv_s(unpack_for_conv(x).detach(), m))
            return hook_s
        if step_mode=='m':
            def hook_m(m: nn.Conv2d, x: tuple, y: torch.Tensor):
                #print(type(x),len(x))
                tmp = unpack_for_conv(x)
                if type(tmp)==tuple:
                #tmp = x
                    #print(type(tmp[0]),len(tmp[0]),tmp[0].shape)
                    x_t = self.expand(tmp[0])
                    x_t2 = self.expand(tmp[1])
                    for i in range(len(x_t)):
                        self.name_records_index[name].append(self.records.__len__())
                        self.records.append(self.cal_sop_Myconv_s((x_t[i].detach(),x_t2[i].detach()), m.conv))
                else:
                    x_t = self.expand(tmp)
                    for i in range(len(x_t)):
                        self.name_records_index[name].append(self.records.__len__())
                        self.records.append(self.cal_sop_conv_s(x_t[i].detach(), m.conv))
            return hook_m        
    ...
    def cal_sop_Mylinear_s(self, x: tuple, m: nn.Linear):
        x_spike = x[1]
        with torch.no_grad():
            sop = (torch.nn.functional.linear(x_spike, torch.ones_like(m.weight), None)).sum()
            tot = (torch.nn.functional.linear(torch.ones_like(x_spike), torch.ones_like(m.weight), None)).sum()
    ...
    def create_hook_Mylinear(self, name, step_mode):
        if step_mode=='s':
            def hook_s(m: nn.Conv2d, x: tuple, y: torch.Tensor):
                self.name_records_index[name].append(self.records.__len__())
                self.records.append(self.cal_sop_linear_s(unpack_for_linear(x).detach(), m.linear))
            return hook_s
        if step_mode=='m':
            def hook_m(m: nn.Conv2d, x: tuple, y: torch.Tensor):
                tmp = unpack_for_conv(x)
                ...
                for i in range(len(x_t)):
                    self.name_records_index[name].append(self.records.__len__())
                    self.records.append(self.cal_sop_Mylinear_s((x_t[i].detach(),x_t2[i].detach()), m.linear))
            return hook_m
