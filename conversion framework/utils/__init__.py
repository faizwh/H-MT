...
class MergeTemporalDim(nn.Module):
    ...
    def forward(self, x_seq: torch.Tensor):
        return x_seq.flatten(0, 1).contiguous()# 保证数据的存储顺序storage和列表形式一致
...
class ExpandTemporalDim(nn.Module):
    ...
    def forward(self, x_seq: torch.Tensor):
        ...
        y_shape.extend(x_seq.shape[1:]) #extend：在列表尾部增添来自其他列表的元素
...
class MyPass(nn.Module):
    def __init__(self, mypass):
        super().__init__()
        self.mypass = mypass
    def forward(self, x,*args):
        if type(x)==tuple:
            return self.mypass(x[0],*args),self.mypass(x[1],*args)
        else:
            return self.mypass(x,*args)
